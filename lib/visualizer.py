...
class Visualizer():
    ...
    def save_current_images(self, epoch, reals, fakes, fixed):
        ...
        Args:
            ...
            fixed ([FloatTensor]): Fixed Fake Image
        """
        vutils.save_image(reals, f'%s/reals{epoch+1}.png' % self.img_dir, normalize=True)
        vutils.save_image(fakes, f'%s/fakes{epoch+1}.png' % self.img_dir, normalize=True)
        vutils.save_image(fixed, f'%s/fixed_fakes_%03d.png' % (self.img_dir, epoch + 1), normalize=True)
