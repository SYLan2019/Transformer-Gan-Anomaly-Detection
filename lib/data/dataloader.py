...
from lib.data.datasets import get_cifar_anomaly_dataset, get_stl_anomaly_dataset
from lib.data.datasets import get_mnist_anomaly_dataset, MVtec
...
def load_data(opt):
    ...
    ## MNIST
        ...
        train_ds, valid_ds = get_stl_anomaly_dataset(train_ds, valid_ds, train_ds.classes.index(opt.abnormal_class))
    elif opt.dataset in ['mvtec']:
        transform = transforms.Compose([transforms.Resize(opt.isize),
                                        transforms.ToTensor(),
                                        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
        train_ds = MVtec("train", opt.dataroot, opt.kind, trans=transform)
        valid_ds = MVtec("test", opt.dataroot, opt.kind, trans=transform)
