...
class MVtec(Dataset):
    def __init__(self, data_type, dir, kinds="bottle", trans=None):
        ...
        for target_kind in os.listdir(dir):
            if target_kind != kinds:
                continue
            path = os.path.join(dir, target_kind, data_type)
            for cur, _, img in os.walk(path):
                for name in os.listdir(cur):
                    if not name.endswith(".png") or name.endswith(".jpg"):
                        break
                    self.img_list.append(os.path.join(cur, name))
                    if cur.endswith("good"):
                        self.label_list.append(0)
                    else:
                        self.label_list.append(1)

    def __getitem__(self, idx):
        cur_path = self.img_list[idx]
